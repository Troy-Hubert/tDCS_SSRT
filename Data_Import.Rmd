...
for(i in 1:length(files)){
  ...
  output <- append(output, paste(files[i]))
}
output <- output[-1]
output
```
...
write_sav(METRIC_FINAL,"~/Downloads/METRIC_DATA.sav")
```
...
data_ref <- read_csv("~/Desktop/Coding/data/SSRT/metric10.csv")
view(data_ref)

## Prepping DF for analyses
#1. Correct the column headers
  head <- data_ref[1,]
  colnames(data_ref) = head
  data <- data_ref[-1,]
  ds_sst <- NA
#2. select and create the new variables we need and put those into a new df
  df <- data_ref %>%
