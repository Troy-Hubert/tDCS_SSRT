...
#3. Create new df used for SSRT analyses / wide data
  ...
  ssrt_df <- df %>%
      mutate(
        ...
        trial = Trial,
        rt = as.double(Go.RT)*.001,
        block = as.integer(block),
        trial = as.integer(trial),
        ssd = as.double(ssd)*.001,
...
integration_adaptiveSSD(ds_sst_1,'condition','rt','response','ssd')


in `df1[, rt_col][nthindex]`:
! Can't subset columns with `nthindex`.
✖ Subscript `nthindex` can't contain missing values.
✖ It has a missing value at location 1.
Backtrace:
 1. SSRTcalc::integration_adaptiveSSD(...)
 8. vctrs (local) `<fn>`()
 9. vctrs:::stop_subscript_missing(i = i, call = call)

## Scoring a participant 
fn_score(subset(ds_sst_1))
...
write.csv(Output_df, "~/Desktop/Coding/data/SSRT_Output/Le_Holy_Grail.csv")
```
